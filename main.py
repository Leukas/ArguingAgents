...
classifier = Classifier(img_shape, 10).to(device)
gan = GAN(generator, discriminator, classifier)

# Load previous save
model_path = './models/mnist/mnist_c.pt'
if os.path.exists(model_path):
	gan.load_state_dict(torch.load(model_path))

# train_cgan(gan, dataloaders['train'], epochs=args.n_epochs, sample_interval=1000, save_path=model_path)
